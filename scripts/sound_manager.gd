...
func _ready() -> void:
	...
	for counter in cutting_counters:
		if counter is CuttingCounter:
			counter.any_cut.connect(_on_any_cut)
	var counters = get_tree().get_nodes_in_group("counters")
	for counter:BaseCounter in counters:
		if counter.has_signal("any_object_placed"):
			counter.any_object_placed.connect(_on_any_object_placed)
...
func _on_delivery_manager_recipe_succeeded():
	play_sound(audio_clip_ref_res.delivery_success, delivery_counter_position)

func _on_delivery_manager_recipe_failed():
...
func _on_player_picked_something(pos):
	play_sound(audio_clip_ref_res.object_pickup, pos)

func _on_any_object_placed(pos):
	play_sound(audio_clip_ref_res.object_drop, pos)


func _on_trash_counter_any_object_trashed(pos):
